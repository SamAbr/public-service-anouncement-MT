...
def find_project_root(start: Path | None = None) -> Path:
    """Walk upwards to the PSA-MT folder: the one holding data/ and notebooks/.

    The repository holds two projects side by side. This returns the
    fine-tuning one, NOT the git root - see REPO_ROOT below for that.
    """
...
def find_repo_root(start: Path | None = None) -> Path:
    """Walk upwards to the git root, the folder holding both project folders."""
    here = (start or Path.cwd()).resolve()
    for candidate in [here, *here.parents]:
        if (candidate / ".git").exists():
            return candidate
        if (candidate / "PSA-MT").is_dir() and (candidate / "corpus_generation").is_dir():
            return candidate
    return here.parent
...
REPO_PATHS = {
    "bible_en_guz_swh.csv": "PSA-MT/data/bible_en_guz_swh.csv",
    "psa_ke_train.csv": "PSA-MT/data/psa_ke_train.csv",
    "psa_ke_test.csv": "PSA-MT/data/psa_ke_test.csv",
    "psa_ke_test_en_guz.csv": "PSA-MT/data/psa_ke_test_en_guz.csv",
    "lughayangu_sentences.csv": "PSA-MT/data/lughayangu_sentences.csv",
    "PSA_KE_Final.csv": "PSA-MT/data/PSA_KE_Final.csv",
    "_PSA_EnGuz.csv": "PSA-MT/data/_PSA_EnGuz.csv",
    # Produced by the other project in this repository.
    "english_psas.csv": "corpus_generation/data/english_psas.csv",
    "psa_parallel_dataset.csv": "corpus_generation/data/psa_parallel_dataset.csv",
}

ROOT = find_project_root()       # PSA-MT/
REPO_ROOT = find_repo_root()     # the git root, holding both projects
INPUTS = ROOT / "data"           # corpora this project trains and tests on
GEN_INPUTS = REPO_ROOT / "corpus_generation" / "data"
...
# These two are outputs of the corpus_generation project, not inputs built here.
...
def download(filename: str, dest: Path | None = None, force: bool = False) -> Path:
    ...
    dest = Path(dest) if dest else REPO_ROOT / REPO_PATHS[filename]
