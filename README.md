 ...
 GPT-5 mini (Azure OpenAI)   constrained few-shot synthesis, 10 parallel workers
   ...
   Validated English public service announcements
         ...
         └──► Ekegusii    via GPT-5 mini, few-shot prompted with hand-curated
                          Ekegusii seed pairs (src/knowledge/ekegusii_fewshot_corpus.py)
```
...
`docs/count_records.py` recomputes every figure in this section from the raw
CSVs, so none of them are copied by hand.
...
python tests/verify_dataset.py        # columns, formats, nulls in the corpus CSVs
...
│
├── english/
│   └── generate_english_only.py    # English PSA synthesis via Azure GPT-5 mini
│
├── src/
│   ├── generator.py                # Concurrent LLM generation orchestrator
│   ├── llm_generator.py            # Azure OpenAI client + validation rewrite loop
│   ├── validator.py                # Length and register constraint enforcement
...
│
├── tests/
│   ├── verify_dataset.py           # Corpus integrity
...
│   ├── psa_parallel_dataset.csv    # Parallel announcement corpus
...
└── docs/
    ...
    ├── count_records.py            # Recomputes the corpus tables from raw CSVs
